...
out <- paste("start_date & bundling. Pearson-Korrelation r =", round(pearson_r, 3))

plot <- ggplot(result, aes(x = start_h, y = bundling)) +
  geom_point() +
  labs(
    title = out,
...
ggsave(
  filename = "startzeit_vs_commits_sample5.pdf",
  plot = plot,
  width = 8,
  height = 5,
  device = cairo_pdf
