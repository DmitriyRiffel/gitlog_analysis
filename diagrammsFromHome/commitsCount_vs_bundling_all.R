...
pearson_tbl <- all_samples_changes %>%
  group_by(sample) %>%
  summarise(
    pearson_r = cor(commits, bundling, method = "pearson", use = "complete.obs"),
    n = sum(complete.cases(commits, bundling)),
    .groups = "drop"
  )
...
# Plot: Farbe = sample
plot_changes <- ggplot(all_samples_changes, aes(x = commits, y = bundling, color = sample)) +
  ...
  labs(
    title = "Gesamte Anzahl von Commits vs. Commit-Bündelung",
...
# PDF speichern
ggsave(
  filename = "total_commits_vs_bundling_all.pdf",
  plot = plot_changes,
  width = 11,
  height = 5,
  device = cairo_pdf
